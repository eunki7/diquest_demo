...
class TextRank:
    ...
    def __init__(self, text, stopwords):
        ...
        if len(stopwords) > 0:
            ...
            # print("_stopwords: {}".format(_stopwords))
    ...
    def summarize(self, count=3):
        ...
        for i in candidates:
            i.weight = "%.6f" % self.pagerank[i]
            # print("text: {}, weight: {}".format(i.text, self.pagerank[i]))

        # return '\n'.join([candidate.text for candidate in candidates])
        return [candidate.text for candidate in candidates], [candidate.weight for candidate in candidates]
